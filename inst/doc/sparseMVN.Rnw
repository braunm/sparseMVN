...
MVN samples, using functions in \pkg{sparseMVN} (\func{rmvn.sparse},
  ...
  \func{dmvnorm}).  Times were collected over 300 replications on a
  ...
  quadratically in the number of blocks.\footnote{As an example, in
    ...
    densities is just over one millisecond using \func{dmvnorm}, but more
    than four milliseconds using \func{dmvn.sparse}.}  This is because the number
  ...
  \emph{nonzero} elements grows linearly.  In this example,
...
Deriving utility from \pkg{sparseMVN} depends on being able to create
and use sparse matrices.  This section describes briefly two other
\proglang{R} packages that may be useful.
...
precision matrix of an MVN approximation. The derivation of the
Hessian might be too tedious or complicated to derive it analytically,
or too large to estimate numerically.  However, for many applications
(e.g., a hierarchical model with conditional independence), it is easy
to identify which elements in the Hessian are nonzero.

The \pkg{sparseHessianFD} package \citep{R_sparseHessianFD} provides
functions to estimate the Hessian numerically, but in a
