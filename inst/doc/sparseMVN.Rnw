...
covariance or precision matrix is sparse.  The \pkg{sparseMVN} package
supplies functions that exploit that sparsity.  The \pkg{sparseMVN}
package provides functions to sample from a
...
those called by the \pkg{Matrix} package, include routines that are
...
$X\sim\MVN{\mu,\Sigma}$, where $\mu\in\Real{K}$ is a vector, and
$\Sigma\in\Real{K\times K}$ is a positive-definite covariance matrix. 

The probability density of $x$ is

\begin{align}
f(x)=(2\pi)^{-\frac{k}{2}}|\Sigma|^{-\frac{1}{2}}
  \exp\left[-\frac{1}{2}\left(x-\mu\right)'\Sigma^{-1}\left(x-\mu\right)\right]
\end{align}

A typical approach to computing this density is
\begin{enumerate}
\item\label{it:dens1} compute a lower triagular matrix root $L$, such that  $\Sigma=LL^\top$;
\item\label{it:dens2} solve for $z$ in the triangular linear system $Lz=\left(x-\mu\right)$;
\item\label{it:dens3} compute $\log|\Sigma|=2\sum_{k=1}^KL_{kk}$; and
\item\label{it:dens4} compute $\log f(x)=-\frac{k}{2}\log(2\pi)-\frac{1}{2}\log|\Sigma|-\frac{1}{2}z'z$
\end{enumerate}

The matrix root $L$ could be generated via an eigenvalue, singular
...
diagonal elements are positive, so we will maintain usage of that
definition. 

Simulating a MVN random variable also requires a Cholesky
decomposition and a triangular matrix multiplication:

\begin{enumerate}
\item \label{it:sim1} compute the Cholesky decomposition $L$, such
  that $\Sigma=LL^\top$;
\item \label{it:sim2} generate $z\in\Real{K}$ by simulating from a
  standard normal distribution $k$ times;
\item \label{it:sim3} compute $x=Lz+\mu$
\end{enumerate}

 We know that $x$ is a sample from $\MVN{\mu,\Sigma}$ because
 \begin{align}
 \EV{Lz}&=0,\text{ and}\\
   \Cov{Lz}&=\EV{Lzz^\top L^\top}
   =L\,\EV{zz^\top}L^\top=LL^\top=\Sigma
 \end{align}

The computational complexity is $\bigO{k^3}$  for a Cholesky
decomposition, and $\bigO{k^2}$ for solving the triangular linear
system \citep{GolubVanLoan1996}, so the total computational cost grows
cubically with the dimension of the random variable.  Additionally, the storage
requirement for $\Sigma$ grows quadratically with $k$.
 
The \func{dmvnorm} and \func{rmvnorm} functions in the \pkg{mvtnorm}
package \citep{R_mvtnorm} generally follow these algorithms for density
computation and random value simulation, respectively. They are
...
Algorithms for computing the MVN density, and simulating MVN random variates, require
only slight modifications when $\Sigma^{-1}$ is available instead of
$\Sigma$. To compute the density:

\begin{enumerate}
\item\label{it:dens1p} compute the Cholesky decomposition of $\Sigma^{-1}$, $\Lambda$, such that  $\Sigma^{-1}=\Lambda\Lambda^\top$;
\item\label{it:dens2p} compute $z = \Lambda^\top\left(x-\mu\right)$;
\item\label{it:dens3p} compute $\log|\Sigma|=-2\sum_{k=1}^K \Lambda_{kk}$; and
\item\label{it:dens4p} compute $\log f(x)=-\frac{k}{2}\log(2\pi)-\frac{1}{2}\log|\Sigma|-\frac{1}{2}z'z$
\end{enumerate}

For simulation,

\begin{enumerate}
\item \label{it:sim1p} compute the Cholesky decomposition of $\Sigma^{-1}$, $\Lambda$, such
  that $\Sigma^{-1}=\Lambda\Lambda^\top$;
\item \label{it:sim2p} generate $z\in\Real{K}$ by simulating from a
  standard normal distribution $k$ times;
\item \label{it:sim3p} solve for $x$ in the triangular linear system $\Lambda^\top\left(x-\mu\right)=z$
\end{enumerate}
...
in a sparse format can be substantion.  If $N=\Sexpr{Q}$, 
...
only need to work with only \Sexpr{nnz2LT} unique values.  The dense
matrix requires \Sexpr{format(object.size(A2),units='Mb')} of RAM,
while a sparse matrix of the \class{dsCMatrix} class requires only \Sexpr{format(object.size(M2),units='Kb')}.
