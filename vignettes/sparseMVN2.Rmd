...
tab1 <- filter(runtimes, stat %in% c("density","rand")) %>%
  ...
  ungroup() %>%
  pivot_wider(names_from = c(pattern, type), values_from = mean_ms)

tab2 <- as_tibble(runtimes) %>%
  filter(stat %in% c("chol","solve")) %>%
  group_by(N, k, stat, pattern, type) %>%
  summarize(mean_ms=mean(time/1000000)) %>%
  pivot_wider(names_from = c(stat, pattern), values_from = mean_ms)
```
...
DF1 <- tab1 %>%
  mutate(stat=fct_recode(stat, random='rand')) %>%
  rename(dense=dense_cov, sparse=sparse_cov) %>%
  pivot_longer(cols=c(dense, sparse), names_to='pattern', values_to='value')
...
DF2 <- tab2 %>%
  ...
  pivot_longer(cols=c(`dense inversion`,`sparse Cholesky`,`dense Cholesky`),
