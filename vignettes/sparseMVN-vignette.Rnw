...
The \proglang{R} package \pkg{mvtnorm} \citep{R_mvtnorm} includes functions for sampling from a multivariate normal (MVN) distribution (\func{rmvnorm}), and for computing the density of an MVN sample (\func{dmvnorm}).  To use these functions, the user provides the covariance matrix as a standard, base \proglang{R} matrix.  This approach can be inefficient when most of the entries in the covariance matrix are zero.  Not only will \proglang{R} store these zeros explicitly, but linear algebra routines (e.g., matrix multiplication, \func{solve}) will treat the zeros like any other number.  Additionally, \func{rmvnorm} factors (e.g., eigenvalue, singular value, or Cholesky) this dense matrix every time the function is called.  If the user has the precision (inverse covariance) matrix instead, a matrix inversion step precedes the calls to either \func{rmvnorm} or \func{dmvnorm}. 

The \pkg{sparseMVN} package provides analogous functions to \func{rmvnorm} and \func{dmvnorm} that are optimized for \emph{sparse} covariance or precision matrices.  The package uses the sparse matrix classes, and associated methods, from the \pkg{Matrix} package \citep{R_Matrix}, which is now one of the ``recommended'' packages in \proglang{R}. In contrast to the functions in \pkg{mvtnorm}, \pkg{sparseMVN} lets the user start with either a covariance or precision matrix.  The user computes a Cholesky decomposition of the matrix first, and passes the decomposition as an argument to either \func{rmvn.sparse} or \func{dmvn.sparse}.  By separating the Cholesky decomposition from the sampling or density functions, the user can call these functions repeatedly, while performing the factorization step only once.  Having the option to work with the precision matrix instead of the covariance avoids the need to explicitly invert the precision matrix when the latter is more readily available.  This option is useful, for example, when the precision of an MVN distribution is determined by the Hessian at the optimum of a log posterior density, as with the Laplace approximation.  More importantly, by using the sparse matrix algorithms in the \pkg{Matrix} package, we dramatically reduce the computational expense of sampling from, and computing densities of, MVN variates of very high dimension.
...
Regardless of whether the user starts with $\Sigma$ or $\Sigma^{-1}$, either the sampling routine or density calculation will involve solving a triangular system of linear equations. Which matrix to use depends mostly on convenience and the task at hand.
