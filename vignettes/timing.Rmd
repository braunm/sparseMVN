...
tmp <- R %>% gather(val, time, c(sample.sparse, sample.dense, density.sparse, density.dense)) %>%
...
First, let's compare times for sampling from an MVN.

```{r}
require(knitr)
tab.samp <- tmp %>% filter(stat %in% c("sample") & prec==FALSE) %>%
    select(-N) %>%
        select(m, p, k, pct.nnz, dense, sparse, cholesky)
kable(tab.samp)
```
...
tab.dens <- tmp %>% filter(stat %in% c("density") & prec==FALSE) %>%
    select(-N) %>%
        select(m, p, k, pct.nnz, dense, sparse)
kable(tab.dens)
```
