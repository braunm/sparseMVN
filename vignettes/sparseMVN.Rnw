...
\func{dmvnorm}.  For each case, we construct a matrix with a
block-arrow structure as in Figure~\ref{fig:blockarrow}.  Cases vary
by the number of blocks in the diagonal (analogous to the number of
heterogeneous units in the binary choice example), the number of
random samples being selected, and whether the underlying matrix is a
...
When a covariance matrix:  Table~\ref{tab:timingCov}.

\begin{table}
  \begin{tabular}{rrrrrr|rrrrrrrr}
    \toprule
&&&&&&\multicolumn{4}{c}{random}&\multicolumn{4}{c}{density}\\
&&&&&pct&\multicolumn{2}{c}{sparse}&\multicolumn{2}{c}{dense}&\multicolumn{2}{c}{sparse}&\multicolumn{2}{c}{dense}\\
...
@ 
\end{tabular}
\caption{Time (milliseconds) to simulate $R$ MVN samples, or compute
  $R$ MVN densities, using sparse (\func{rmvn.sparse},
  \func{dmvn.sparse}) or dense (\func{rmvnorm}, \func{dmvnorm})
  methods, for a covariance matrix with a block-arrow structure. $N$ is
number of blocks, $k$ is the size of the block, nnz is the number of
non-zeros in the lower triangle of the matrix, and pct.nnz is the
percentage of elements that are nonzero (a measure of sparsity). Means
and standard deviations are across XX replications.}\label{tab:timingCov}
\end{table}

When a precision matrix:  Table~\ref{tab:timingPrec}.
...
\caption{Time (milliseconds) to simulate $R$ MVN samples, or compute
  $R$ MVN densities, using sparse (\func{rmvn.sparse},
  \func{dmvn.sparse}) or dense (\func{rmvnorm}, \func{dmvnorm})
  methods, for a precision matrix with a block-arrow structure. See
  caption to Table~\ref{tab:timingCov} for definitions of columns.}\label{tab:timingPrec}
\end{table}
