...
\begin{table}
  \begin{tabular}{rrrrrrr}
    \toprule
&&&\multicolumn{2}{c}{density}&\multicolumn{2}{c}{random}\\
&&&dense&sparse&dense&sparse
<<echo=FALSE,results='asis'>>=
filter(runtimes, step %in% c("rand","density")) %>%
    mutate(type=ifelse(prec,"cov","prec")) %>%
    select(-s, -k, -prec) %>%
    gather(stat,value, c(mean,sd)) %>%
    reshape2::dcast(type+N+stat~step+storage) %>%
    xtable(digits=c(rep(0,4),rep(1,4))) %>%
...
number of blocks; see Table~\ref{tab:cases} for dimensions and
sparsity of each matrix. Means and standard deviations are across XX replications.}\label{tab:timing}
\end{table}
